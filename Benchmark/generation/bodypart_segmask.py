...
def segmask_cardiomegaly(args, dicom, target_df, save_base_dir):
    ...
    plt.imshow(cxr)
    plt.plot([xmin_lung, xmin_lung], [ymin_lung, ymax_lung], color="red", linewidth=1.5)
    plt.plot([xmax_lung, xmax_lung], [ymin_lung, ymax_lung], color="red", linewidth=1.5)
    plt.plot([xmin_lung, xmax_lung], [ymean_lung, ymean_lung], color="red", linewidth=1.5)
    plt.axis('off')
    plt.savefig(f'{save_path}/{dicom}.png', bbox_inches='tight')
    plt.close()
...
def segmask_descending_aorta(args, dicom, target_df, save_base_dir):
    ...
    ymin_start = target_df['ymin_start'].values[0]
    ymin_end = target_df['ymin_end'].values[0]
...
def segmask_aortic_knob(args, dicom, target_df, save_base_dir):
    ...
    y_max = target_df['y_max'].values[0]
    ymin_desc = target_df['ymin_desc'].values[0]
    ysub_desc = target_df['ysub_desc'].values[0]
    ...
    ymax_desc_sub = target_df['ymax_desc_sub'].values[0]
    mask_aortic_arch[ymax_desc_sub:] = 0  # y axis - unexpected region에 있는 blob 제거
...
def segmask_inclusion(args, dicom, target_df, save_base_dir):
    ...
    mask_rlung = cv2.imread(fname_rlung, cv2.IMREAD_GRAYSCALE)
    mask_llung = cv2.imread(fname_llung, cv2.IMREAD_GRAYSCALE)
    mask_rdp = cv2.imread(fname_rdp, cv2.IMREAD_GRAYSCALE)
    mask_ldp = cv2.imread(fname_ldp, cv2.IMREAD_GRAYSCALE)
...
def segmask_inspiration(args, dicom, target_df, save_base_dir):
    ...
    mask_dp = np.array(Image.open(fname_dp))
...
def segmask_mediastinal_widening(args, dicom, target_df, save_base_dir):
    ...
    mask_medic = cv2.imread(fname_medic, 0)
...
def segmask_projection(args, dicom, target_df, save_base_dir):
    ...
    mask_r_scapular = cv2.imread(fname_r_scapular, 0) // 255
...
def segmask_trachea(args, dicom, target_df, save_base_dir):
    ...
    midline_x_min = target_df['midline_x_min'].values[0]
    midline_x_max = target_df['midline_x_max'].values[0]
    y_min = target_df['y_min'].values[0]
    y_max = target_df['y_max'].values[0]
